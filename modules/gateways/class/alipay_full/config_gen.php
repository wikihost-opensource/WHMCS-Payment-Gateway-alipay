...
class alipayfull_config{
    ...
    function get_configuration ($params){
        ...
        $type = isset($params['apitype']) ? $params['apitype'] : Capsule::table("tblpaymentgateways")->where("gateway","alipay_full")->where("setting","apitype")->first()->value;
        $skintype = isset($params['skintype']) ? $params['skintype'] : Capsule::table("tblpaymentgateways")->where("gateway","alipay_full")->where("setting","skintype")->first()->value;
        if (empty($type)){
            $extra_config = [
                "notice" => [
                'FriendlyName' => '温馨提示',
